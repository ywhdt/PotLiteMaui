...
- 鼠标附近弹出翻译结果窗口，支持复制、固定和自动隐藏
- 托盘常驻，关闭窗口时可隐藏到托盘
- 可选开机启动
- 历史记录默认开启，可设置保留条数、删除单条或清空全部
- API Key 存入系统安全凭据，不写入普通 JSON 设置文件

## 翻译引擎

- Google 翻译：默认引擎，使用网页接口，不需要 API Key；该接口是实验性方案。
- Bing 词典：使用 Azure Translator Dictionary Lookup，需要 Azure Translator key、endpoint、region；适合单词或短语。
...
- Google 和 Bing 词典仍会使用 UI 中的语言设置。

## 设计分层

- `Models`：设置、翻译结果、词典结果、历史记录模型
- `Services/Translation`：Google、Azure 词典、OpenAI provider 和 provider registry
